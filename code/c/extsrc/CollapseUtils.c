...
//globals
static 	Int32 	g_CollapseAPIVersion = 0;
 ...
 *
 ***********************************************************************/
Boolean CollapseResizeForm(FormType *frmP, Boolean isModal, Boolean moveForm, 
			Int16* returnOffsetX, Int16* returnOffsetY)
{
	...
	if (g_CollapseAPIVersion == 0) return false;
	...
	if (offsetCoordsX || offsetCoordsY)
	{
		...
		if (moveForm) {
			...
			formBounds.topLeft.x += offsetCoordsX;
			formBounds.topLeft.y += offsetCoordsY;
...
//	return false;
	return CollapseCheckForPin10NeedToRedraw();
